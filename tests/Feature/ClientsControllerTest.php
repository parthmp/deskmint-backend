...
class ClientsControllerTest extends TestCase
{
	...
	public function test_if_it_fetches_clientS_custom_fields_with_testing_valid_default_values(): void{
		...
		foreach($custom_field_types as $field_type){
			...
			}else if($field_type->input_type === config('global.field_types')[4]){ /* select */
				$default_value = 'two';
				$options = ['one', 'two', 'three'];
			}else if($field_type->input_type === config('global.field_types')[5]){ /* number */
				$default_value = '123';
			}else if($field_type->input_type === config('global.field_types')[6]){ /* date */
		...
		foreach($response as $r_field){
			...
			}else if($r_field['custom_field_type']['input_type'] === config('global.field_types')[4]){ /* select */
				$this->assertEquals('two', $r_field['value']);
			}else if($r_field['custom_field_type']['input_type'] === config('global.field_types')[5]){ /* number */
				$this->assertEquals('123', $r_field['value']);
			}else if($r_field['custom_field_type']['input_type'] === config('global.field_types')[6]){ /* date */
				$this->assertEquals('2025-01-21', $r_field['value']);
			}else if($r_field['custom_field_type']['input_type'] === config('global.field_types')[6]){ /* time */
				$this->assertEquals('01:45 AM', $r_field['value']);
