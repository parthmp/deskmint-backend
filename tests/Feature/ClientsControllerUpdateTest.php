...
class ClientsControllerUpdateTest extends TestCase{
	...
	private function updatedData($currency, $country, $industry, $company_id, $custom_fields = []){
		return $this->clientStoreData($currency, $country, $industry, $company_id, $custom_fields, [
			'personal_info.last_name.value'		=>	'test lastname u',
			'personal_info.client_company_name'	=>	'client company u',
	...
	public function test_if_it_updates_the_client_with_no_custom_fields123():void{
		...
		$post_data = $this->updatedData($currency, $country, $industry, $company_id);
		...
		$this->assertEquals('client company u', $client->client_company_name);
		$this->assertEquals('ind u', $client->peppol_identifier);
		$this->assertEquals('sch u', $client->peppol_scheme);
	...
	public function test_if_it_updates_the_client_with_no_custom_fields123_with_peppol() : void{
	...
	public function test_if_it_updates_the_client_with_all_custom_fields_and_all_removals():void{
		...
		/* remove some custom fields */
		$random_custom_field_ids = ClientsCustomField::pluck('id')->toArray();
		$this->deleteClientsCustomFields($random_custom_field_ids, $c['headers'], $company_id);
		$temp_order['order'] = 1;
