...
trait ArrangedColumns{
	...
	private function validatePostedColumns(Request $request){
	...
	protected function saveArrangedColumnsData(Request $request, string $custom_fields_model, string $feature_name, string $original_table, string $type) : Response{
		
		$validated = $this->validatePostedColumns($request);
		if($validated !== null){
			return $validated;
		}
